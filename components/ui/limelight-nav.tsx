...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center justify-between h-16 rounded-lg border px-2 ${className || 'bg-card'}`}>
      {items.map(({ id, icon, label, onClick }, index) => (
          <a
            key={id}
            ref={el => { navItemRefs.current[index] = el; }}
            className={`relative z-20 flex h-full cursor-pointer items-center justify-center flex-1 ${iconContainerClassName}`}
      ...
      <div
        ref={limelightRef}
        className={`absolute top-0 z-10 w-16 h-[4px] rounded-full bg-primary shadow-[0_30px_15px_var(--primary)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-0 top-[4px] w-full h-12 bg-gradient-to-b from-primary/30 to-transparent pointer-events-none" style={{ clipPath: 'polygon(0% 0%, 100% 0%, 85% 100%, 15% 100%)' }} />
      </div>
    </nav>
  );
};
